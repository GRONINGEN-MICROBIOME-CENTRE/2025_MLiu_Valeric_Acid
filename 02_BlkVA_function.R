...
metadata_VA_1mM_24h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_1mM_24h.csv", header = TRUE, row.names = 1)
count_data_VA_1mM_24h <- count_data[, rownames(metadata_VA_1mM_24h), drop = FALSE]

metadata_VA_3mM_24h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_3mM_24h.csv", header = TRUE, row.names = 1)
count_data_VA_3mM_24h <- count_data[, rownames(metadata_VA_3mM_24h), drop = FALSE]

metadata_VA_10mM_24h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_10mM_24h.csv", header = TRUE, row.names = 1)
count_data_VA_10mM_24h <- count_data[, rownames(metadata_VA_10mM_24h), drop = FALSE]

# ----------------------------------------------------------------------------------------------------------------------
metadata_VA_0.3mM_6h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_0.3mM_6h.csv", header = TRUE, row.names = 1)
count_data_VA_0.3mM_6h <- count_data[, rownames(metadata_VA_0.3mM_6h), drop = FALSE]

metadata_VA_1mM_6h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_1mM_6h.csv", header = TRUE, row.names = 1)
count_data_VA_1mM_6h <- count_data[, rownames(metadata_VA_1mM_6h), drop = FALSE]

metadata_VA_3mM_6h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_3mM_6h.csv", header = TRUE, row.names = 1)
count_data_VA_3mM_6h <- count_data[, rownames(metadata_VA_3mM_6h), drop = FALSE]

metadata_VA_10mM_6h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_10mM_6h.csv", header = TRUE, row.names = 1)
count_data_VA_10mM_6h <- count_data[, rownames(metadata_VA_10mM_6h), drop = FALSE]
# ------------------------------------------------------------------------------------------------------------------------------------------------------------

run_blkVA_basic(count_data_VA_0.3mM_24h, metadata_VA_0.3mM_24h, conc_label = "0.3", time_label = "24h")
run_blkVA_basic(count_data_VA_1mM_24h,   metadata_VA_1mM_24h,   conc_label = "1",   time_label = "24h")
run_blkVA_basic(count_data_VA_3mM_24h,   metadata_VA_3mM_24h,   conc_label = "3",   time_label = "24h")
run_blkVA_basic(count_data_VA_10mM_24h,   metadata_VA_10mM_24h,   conc_label = "10",   time_label = "24h")

run_blkVA_basic(count_data_VA_0.3mM_6h,    metadata_VA_0.3mM_6h,    conc_label = "0.3",   time_label = "6h")
run_blkVA_basic(count_data_VA_1mM_6h,    metadata_VA_1mM_6h,    conc_label = "1",   time_label = "6h")
run_blkVA_basic(count_data_VA_3mM_6h,    metadata_VA_3mM_6h,    conc_label = "3",   time_label = "6h")
run_blkVA_basic(count_data_VA_10mM_6h,    metadata_VA_10mM_6h,    conc_label = "10",   time_label = "6h")

# ------------------------------------------------------------------------------------------------------------------------------------------------------------

run_blkVA_basic <- function(count_data, metadata, conc_label, time_label,
                            out_dir = "../results/Orga_bulkrna/VA_0810/02_BlkVA") {
  # 对齐
  ...
  tag <- paste0("BlkVA_", conc_label, "mM_", time_label)
  ...
  up_genes <- deg_results_df %>% dplyr::filter(FDR < deg_threshold & logFC > logFC_threshold) %>% dplyr::pull(SYMBOL)
  down_genes <- deg_results_df %>% dplyr::filter(FDR < deg_threshold & logFC < -logFC_threshold) %>% dplyr::pull(SYMBOL)
  all_degs <- c(up_genes, down_genes)
  # ---- KEGG (All) ----
  if (length(all_degs) >= 3) {
    all_entrez <- bitr(all_degs, fromType="SYMBOL", toType="ENTREZID", OrgDb=org.Hs.eg.db)
    if (!is.null(all_entrez) && nrow(all_entrez) >= 3) {
      kegg_res <- enrichKEGG(gene = all_entrez$ENTREZID, organism = "hsa", pvalueCutoff = 0.05)
      if (!is.null(kegg_res) && nrow(as.data.frame(kegg_res)) > 0) {
        pkegg <- dotplot(kegg_res, showCategory=20, title="KEGG Pathway Enrichment")
        print(pkegg)  # 你原来这里只print不保存，我保持不变
  ...
  if (length(up_genes) >= 3) {
    ...
    if (!is.null(up_entrez) && nrow(up_entrez) >= 3) {
      kegg_res_up <- enrichKEGG(gene = up_entrez$ENTREZID, organism = "hsa", pvalueCutoff = 0.05)
      if (!is.null(kegg_res_up) && nrow(as.data.frame(kegg_res_up)) > 0) {
        ...
        print(pkegg_up)
        ggsave(filename = file.path(out_dir, paste0("KEGGup_", tag, ".pdf")),
               plot = pkegg_up, width = 8, height = 8, dpi = 300)
  ...
  if (length(down_genes) >= 3) {
    ...
    if (!is.null(down_entrez) && nrow(down_entrez) >= 3) {
      kegg_res_down <- enrichKEGG(gene = down_entrez$ENTREZID, organism = "hsa", pvalueCutoff = 0.2)
      if (!is.null(kegg_res_down) && nrow(as.data.frame(kegg_res_down)) > 0) {
        ...
        print(pkegg_down)
        ggsave(filename = file.path(out_dir, paste0("KEGGdown_", tag, ".pdf")),
               plot = pkegg_down, width = 8, height = 8, dpi = 300)
