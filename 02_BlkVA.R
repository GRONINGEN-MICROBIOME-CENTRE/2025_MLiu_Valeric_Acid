...
metadata_VA_0.3mM_24h <- read.csv("../results/Orga_bulkrna/02_BlkVA/metadata_VA_0.3mM_24h.csv", header = TRUE, row.names = 1)

# 从 count_data 中保留指定列
count_data_VA_0.3mM_24h <- count_data[, rownames(metadata_VA_0.3mM_24h), drop = FALSE]

# ----------------------------------------------------------------------------------------------------------------------
# DGE
metadata <- metadata_VA_0.3mM_24h
count_data <- count_data_VA_0.3mM_24h
...
write.csv(deg_results_df, file = "../results/Orga_bulkrna/VA_0810/02_BlkVA/DEG_results_VA_0.3mM_24h.csv", row.names=TRUE)
...
pp <- ggplot(pca_df, aes(x = PC1, y = PC2, color = CellLine, shape = Concentration)) +
  ...
  scale_shape_manual(values = c(16, 17)) +  # Different shapes for Concentration
...
ggsave(filename = "../results/Orga_bulkrna/VA_0810/02_BlkVA/PCA_BlkVA_0.3mM_24h.pdf", plot = pp, width = 8, height = 6, dpi = 300)
...
ggsave(filename = "../results/Orga_bulkrna/VA_0810/02_BlkVA/Volcano_BlkVA_0.3mM_24h.pdf", plot = pv, width = 7, height = 6, dpi = 300)
### Heatmap
# 提取显著差异基因
...
ggsave(filename = "../results/Orga_bulkrna/VA_0810/02_BlkVA/Heatmap_BlkVA_0.3mM_24h.pdf", plot = ph, width = 8, height = 6, dpi = 300)
...
ggsave(filename = "../results/Orga_bulkrna/VA_0810/02_BlkVA/KEGGup_BlkVA_0.3mM_24h.pdf", plot = pkegg_up, width = 8, height = 8, dpi = 300)
# 下调基因
...
ggsave(filename = "../results/Orga_bulkrna/VA_0810/02_BlkVA/KEGGdown_BlkVA_0.3mM_24h.pdf", plot = pkegg_down, width = 8, height = 8, dpi = 300)
